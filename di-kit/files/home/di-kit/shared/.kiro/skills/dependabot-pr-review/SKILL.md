...
2. Try direct API calls first (ambient credentials via sbx proxy)
3. Fall back to `gh` CLI if API ping fails
4. Validate empty PR lists against `open_issues_count` — never trust a silent `[]`
5. If all methods fail, report the permission gap to the user
